...
for (let password of input) {
	...
	let policyData = splitted[0].split(" ");
	let limits = policyData[0].split("-");

	let min = Number(limits[0]);
	let max = Number(limits[1]);
	let letter = policyData[1];

	let finalPassword = splitted[1].trim();

	let matchArray = finalPassword.match(new RegExp(letter, "g"));

	if (matchArray && matchArray.length >= min && matchArray.length <= max) {

		validPasswordCount++;

	}

	console.log(min, max, letter, finalPassword, (matchArray && matchArray.length >= min && matchArray.length <= max));

}

console.log("Parte 1: " + validPasswordCount);
...
for (let password of input) {
	...
	let min = Number(limits[0]) - 1;
	let max = Number(limits[1]) - 1;
	let letter = policyData[1];

	let finalPassword = splitted[1].trim();

	if ((finalPassword.charAt(min) == letter && finalPassword.charAt(max) != letter) || (finalPassword.charAt(min) != letter && finalPassword.charAt(max) == letter)) {

		validPasswordCount++;

	}

	console.log(min, max, letter, finalPassword, (finalPassword.charAt(min) == letter || finalPassword.charAt(max) == letter));

}
console.log("Parte 2: " + validPasswordCount);