...
function serchValidBags(colors: string[], inputs: string[]) {

	for (let input of inputs) {

		let newColorsToSearch: string[] = [];

		let bagColor: string = input.split(" bags contain")[0];

		let containedColors: string[] = input.split(" bags contain")[1].replace(/\d /g, "").replace(/ bags?\.?/g, "").trim().split(", ");

		let validBag: boolean = true;

		for (let color of colors) {

			if (!containedColors.includes(color)) {
				...
				newColorsToSearch.push(bagColor);
		...
		if (validBag) {
			...
			if (!validBags.includes(bagColor)) {

				validBags.push(bagColor)

				serchValidBags(newColorsToSearch, inputs);
...
function serchValidBagsAndCount(color: string, inputs: string[], total: number) {
	...
	for (let input of inputs) {
		...
		if (bagColor == color) {
			...
			let numbers: number[] = input.split(" bags contain")[1].replace(/\D/g, "").split("").map(v => Number(v));
			let containedColors: string[] = input.split(" bags contain")[1].replace(/\d /g, "").replace(/ bags?\.?/g, "").trim().split(", ");

			total = 1;

			for (let containedColor of containedColors) {

				if (numbers[containedColors.indexOf(containedColor)]) {

					total += numbers[containedColors.indexOf(containedColor)] * serchValidBagsAndCount(containedColor, inputs, total);
...
function part1() {
	...
	serchValidBags(["shiny gold"], inputs);
	console.log(validBags, validBags.length);

}

function part2() {

	let total = serchValidBagsAndCount("shiny gold", inputs, 0);
